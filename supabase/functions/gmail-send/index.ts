...
import { refreshAccessToken, sendGmailMessage } from '../_shared/google.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const rawBase64url = String(body?.raw ?? '').trim();

    if (!sessionId) return jsonResponse(req, { message: 'sessionId requerido' }, 400);
    if (!rawBase64url) return jsonResponse(req, { message: 'raw (base64url MIME) requerido' }, 400);
    ...
    try {
      result = await sendGmailMessage(authState.accessToken, rawBase64url);
    } catch (err) {
